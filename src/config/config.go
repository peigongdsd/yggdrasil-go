...
type NodeConfig struct {
	...
	HillTweakMs         int64                      `json:",omitempty" comment:"Per-node hillTweak in milliseconds. This value is exchanged during peering and summed with the peer's value to bias routing cost. Default is 0."`
	OrgPubKey           string                     `json:",omitempty" comment:"Organization public key (ed25519) as hex. Used to verify org-issued node certificates."`
	OrgCert             string                     `json:",omitempty" comment:"Organization-issued certificate as hex. If present and valid, it can authenticate this node during peering."`
